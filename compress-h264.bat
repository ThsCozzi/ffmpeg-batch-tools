...
REM 1 = yes, delete the original file only if the compressed file was created.
...
for %%F in (%EXTENSIONS%) do (
    if exist "%%~fF" (
        ...
        ffmpeg -hide_banner -y -i "%%~fF" -map 0:v:0 -map 0:a? -c:v libx264 -b:v 2000k -preset slow -c:a aac -b:a 128k -movflags +faststart "!OUT!"
