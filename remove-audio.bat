...
REM Delete the original video after successfully creating the muted copy?
REM 0 = no, keep the original file. This is the safest default.
REM 1 = yes, delete the original file only if the muted file was created.
...
for %%F in (%EXTENSIONS%) do (
    if exist "%%~fF" (
        set /a COUNT+=1
        set "OUT=%DEST%\%%~nF_muted.mp4"
        echo ----------------------------------------
        echo Removing audio from: %%~nxF

        ffmpeg -hide_banner -y -i "%%~fF" -map 0:v:0 -c:v copy -an "!OUT!"
        ...
        if exist "!OUT!" (
            ...
            echo [FAILED] Could not remove audio: %%~nxF
            set /a FAILED+=1
        )
    )
)
